...
export async function getConfig (): Promise<Config> {
  ...
  try {
    db = {
      database: env.postgres.db || (await fs.readFile(env.postgres.dbFile, 'utf8')).toString().strim(),
      host: env.postgres.host,
      password: env.postgres.password || (await fs.readFile(env.postgres.passwordFile, 'utf8')).toString().strim(),
      port: env.postgres.port,
      user: env.postgres.user || (await fs.readFile(env.postgres.userFile, 'utf8')).toString().strim()
    }
  } catch (error) {
