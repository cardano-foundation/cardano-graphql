...
describe('assets', () => {
  ...
  it('can return information on assets by fingerprint', async () => {
    const result = await client.query({
      ...
      variables: {
        where: {
          fingerprint: { _eq: 'asset1325zj0yt3dxccmxk8g9429l4krg4yz6tyaxpnk' }
        }
      }
    })
  ...
  it('can return information on assets by assetId', async () => {
    const result = await client.query({
      ...
      variables: {
        where: {
          _and: [{
            assetId: { _eq: '34d1adbf3a7e95b253fd0999fb85e2d41d4121b36b834b83ac069ebb41474958' }
          }]
        }
      }
    })
    ...
    expect(assets[0].description).toBe('SingularityNET AGIX Token')
    expect(assets[0].fingerprint).toBe('asset1ge6ra7ueemu0gtst0txjeal5t6m5j3y2jwngl7')
    ...
    expect(assets[0].policyId).toBe('34d1adbf3a7e95b253fd0999fb85e2d41d4121b36b834b83ac069ebb')
