...
export class TransactionRepository extends Repository<TxDataModel> {
  ...
  static async transform (dataSet: Promise<TxDataModel[]>): Promise<Transaction[]> {
    return (await dataSet).map(this.castTransaction)
  }

  static castTransaction (data: TxDataModel): Transaction {
    delete data.id
