import * as path from 'path'
import { Config as ServerConfig } from './Server'
import { buildHasuraSchema } from './lib/buildHasuraSchema'
import { hasuraResolvers, mockedResolvers, scalarResolvers } from './resolvers'
import { StakePoolMetadataRepository } from './StakePoolMetadataRepository'

export async function getConfig (): Promise<ServerConfig> {
  const {
    apiPort,
    hasuraUri,
    stakePoolMetaRepoUri,
    queryDepthLimit,
    tracing
  } = filterAndTypecastEnvs(process.env)

  return {
    apiPort: apiPort || 3100,
    context: hasuraUri ? await buildContext(hasuraUri, stakePoolMetaRepoUri) : undefined,
    queryDepthLimit: queryDepthLimit || 10,
    resolvers: Object.assign({}, scalarResolvers, hasuraUri ? hasuraResolvers : mockedResolvers),
    tracing
  }
}

function filterAndTypecastEnvs (env: any) {
  const {
    API_PORT,
    HASURA_URI,
    QUERY_DEPTH_LIMIT,
    STAKE_POOL_META_REPO_URI,
    TRACING
  } = env
  return {
    apiPort: Number(API_PORT),
    hasuraUri: HASURA_URI,
    queryDepthLimit: Number(QUERY_DEPTH_LIMIT),
    stakePoolMetaRepoUri: STAKE_POOL_META_REPO_URI,
    tracing: Boolean(TRACING)
  }
}

async function buildContext (hasuraUri: string, stakePoolMetaRepoUri?: string) {
  const hasura = await buildHasuraSchema(hasuraUri)
  const stakePoolMetaRepo = stakePoolMetaRepoUri ? await StakePoolMetadataRepository({
    localPath: path.join(__dirname, '../state/stake_pool_metadata_repository'),
    remoteUri: stakePoolMetaRepoUri
  }) : undefined
  return () => ({ hasura, stakePoolMetaRepo })
}
